    ...
    return (
        <footer className='fixed bottom-0 inset-x-0 text-center sm:text-lg max-w-[600px] mx-auto'>
            <div className="flex flex-row gap-4 justify-between bg-slate-900 p-3 my-3 mx-4 text-yellow-300 rounded-xl">
                <Link href={'/boulders'}><h1 className={'p-1 ms-2 border-2 rounded-lg w-20 ' + (pathname === '/boulders' ? ' border-yellow-300 ' : ' border-slate-900')}>boulders</h1></Link>
                ...
                <Link href={'/profile'}><h1 className={'p-1 me-2 border-2 rounded-lg w-20' + (pathname === '/profile' ? ' border-yellow-300 ' : ' border-slate-900')}>info</h1></Link>
            </div>
    </footer>
    )
}
